...
struct vec<4, T>
{
    vec() { for (size_t i = 4; i--; data_[i] = T()); }
    T& operator[](const size_t i) { assert(i < 4 && i >= 0); return data_[i]; }
    const T& operator[](const size_t i) const { assert(i < 4 && i >= 0); return data_[i]; }
